...
def calculate(expr):
	...
	alphabet = "abcdefghijklmnopqrstuvwxyzπ"
	operands = "+-*/^"
	dijits = "0123456789.~"
	data = []
	num = ''
...
# print(calculate("3.141592653589793+3.141592653589793"))
...
def solve(expr):
	...
	for i in s:                    # чтение строки и запись выражения в скобках в отдельную переменную, причем
		...
		if cnt_left != cnt_right:
			...
			# print(function, brakets_expr[1:])
			# print(function, solve(brakets_expr[1:]))
			s = s.replace(f"{function}{brakets_expr})", convert(mathoper(function, solve(brakets_expr[1:]))), 1)
			# print(s)
			# exit()
	return solve(s)
